...
public class HttpRequest extends HttpBase<HttpRequest> {
	private static final String BOUNDARY = "--------------_" + SecureUtil.simpleUUID();
	...
	public HttpRequest body(JSONObject json) {
		...
		if(StringUtil.isNotBlank(this.charset)){
			contentTypeJson = StringUtil.format("{};charset={}", contentTypeJson, this.charset);
		}
	...
	public HttpRequest body(byte[] bodyBytes) {
		return body(StringUtil.str(bodyBytes, this.charset));
	}
	// ---------------------------------------------------------------- Body end
	...
	private void writeFileForm(OutputStream out) throws IOException {
		...
		for (Entry<String, File> entry : this.fileForm.entrySet()) {
			file = entry.getValue();
			StringBuilder builder = StringUtil.builder().append("--").append(BOUNDARY).append(StringUtil.CRLF);
